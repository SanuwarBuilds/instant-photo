...
def gemini_analyze():
    try:
        ...
        # Call Google Gemini API via REST API
        url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={gemini_key}"
        headers = {"Content-Type": "application/json"}
        
        prompt = (
            "Analyze this photo for suitability as a professional passport photo. "
            "Check face alignment (front-facing, centered), lighting (no harsh shadows, good brightness), "
            "expression (neutral or slight smile, eyes open), and clothing. "
            "Provide 1 or 2 simple, direct feedback sentences in Hindi/Hinglish (e.g. 'Face bilkul front aur lighting achhi hai.' or 'Face par side shadow hai, dark area hai.'). "
            "Keep the response brief, friendly, and under 25 words."
        )
        ...
        if res.status_code != 200:
            return jsonify({"error": f"Gemini API returned status {res.status_code}: {res.text}"}), 502
        ...
        try:
            ...
            return jsonify({"error": f"Failed to parse Gemini response structure: {response_data}"}), 500
